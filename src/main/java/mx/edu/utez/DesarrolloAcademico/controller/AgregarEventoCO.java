    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (errorMsg == null) {
                ...
                String mensajeAmigable = "No se pudo guardar el evento.";

                // Mapeo del error ORA-12899 (Oracle) y Data Truncation (MySQL)
                if (errorMsg.contains("ORA-12899") || errorMsg.contains("12899") || errorMsg.contains("Data truncation") || errorMsg.contains("1406")) {
                    mensajeAmigable = "No se pudo agregar: Uno de los campos (como la descripción) excede el tamaño máximo de caracteres permitido.";
                } else {
                    mensajeAmigable = "No se pudo guardar: " + errorMsg;
                }

                out.write("{\"success\": false, \"message\": \"" + escapar(mensajeAmigable) + "\"}");
            }
        } catch (Exception e) {
            e.printStackTrace();
            response.setStatus(HttpServletResponse.SC_OK);

            String mensajeError = "Error inesperado al intentar guardar el evento.";
            ...
            if (msg != null && (msg.contains("ORA-12899") || msg.contains("12899") || msg.contains("Data truncation") || msg.contains("1406"))) {
                mensajeError = "No se pudo agregar: Uno de los campos excede el tamaño máximo de caracteres permitido.";
            } else if (msg != null && !msg.trim().isEmpty()) {
                mensajeError = "Error en el servidor: " + msg;
            }
